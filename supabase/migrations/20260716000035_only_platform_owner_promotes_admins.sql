-- Only the platform owner can create a new family/group admin. A family admin
-- can manage members inside their own group, but cannot delegate admin power.
...
begin
  ...
  if public.has_role('super_admin') then
    ...
    if v_role_name = 'super_admin' then
      raise insufficient_privilege using message = 'only the platform owner can promote admins';
    end if;
